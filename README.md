...
**Candidate:** Kofoworola Ajani
**Live Site:** [kofoworola-worknoon.htwcleaning.com](https://kofoworola-worknoon.htwcleaning.com/)
**LinkedIn:** [Kofoworola Taiwo Ajani](https://www.linkedin.com/in/kofoworola-taiwo-ajani-005468240/)
**Instagram:** [@kofoworola.aj](https://www.instagram.com/kofoworola.aj/)
...
|---|---|---|
| A | [Live Site](https://kofoworola-worknoon.htwcleaning.com/) | Responsive WordPress landing page built with Elementor |
| B | [`/schema`](./schema/) | JSON-LD structured data for Organisation, Person, and Website |
| C | [`knowledge-panel-strategy.md`](./docs/knowledge-panel-strategy.md) | Google Knowledge Panel optimisation strategy |
| D | [`seo-diagnosis.md`](./docs/seo-diagnosis.md) | Technical SEO troubleshooting guide |
| E | [`short-answers.md`](./docs/short-answers.md) | Answers to four technical assessment questions |
| F | [`README.md`](./README.md) | Project reflection and system thinking |
...
### `person-schema.json`
Declares the founder (Kofoworola Ajani) and links her to the organisation via the `worksFor` property, creating a person-to-organisation relationship Google can traverse in the Knowledge Graph.
...
**Deployment method:** Injected via Insert Headers and Footers plugin using `<script type="application/ld+json">` tags in the site header.

**Validation:** Tested at [search.google.com/test/rich-results](https://search.google.com/test/rich-results)

## Section C — Knowledge Panel Strategy

See [`knowledge-panel-strategy.md`](./docs/knowledge-panel-strategy.md)
...
See [`seo-diagnosis.md`](./docs/seo-diagnosis.md)

Scenario: *A new Worknoon website is not indexing after sitemap submission.*
...
6. Search Console debugging steps — six-step sequence with a full coverage error table

## Section E — Short Answer Questions

See [`short-answers.md`](./docs/short-answers.md)

1. **Knowledge Graph vs Knowledge Panel** — the database vs the display layer
2. **How Google determines entity identity** — schema, `sameAs`, NAP consistency, co-citation, user behaviour
...
The brief was to build a functional, optimised WordPress landing page demonstrating real-world capability across development, technical SEO, structured data, and system thinking. The challenge was not to build something that looks good — it was to build something that is technically sound, semantically structured, and genuinely ready for search engine discovery.
...
| WP-Optimize over LiteSpeed Cache | All-in-one database cleaning, image compression, and caching in a single lightweight plugin |
| Site Kit by Google | Officially supported Google plugin that connects GA4 and Search Console directly — satisfies the analytics requirement with zero configuration risk |
| Real testimonials over lorem ipsum | Lorem ipsum on a live site signals unfinished work; realistic Lagos-based client stories add immediate credibility |
...
- **Speed vs visual richness** — Elementor adds page weight; mitigated with WP-Optimize caching and image compression but a hand-coded page would score higher on Core Web Vitals
...
**Sitemap URL mismatch** — Rank Math generates `/sitemap_index.xml` but Search Console initially showed an error against `/sitemap.xml`. Resolved by submitting the correct URL directly and confirming `Success` status.
...
*All schema files, strategy documents, and theme/plugin files are included in this repository.*
*Live site: [kofoworola-worknoon.htwcleaning.com](https://kofoworola-worknoon.htwcleaning.com/)*
